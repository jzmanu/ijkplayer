...
static int read_thread(void *arg)
{
    ...
    // 创建AVFormatContext，指定打开流、关闭流的默认函数等
    ...
    // scan_all_pmts是mpegts的一个选项，这里在没有设定该选项的时候，强制设为1。最后执行avformat_open_input。
    ...
    // 是否配置了跳过帧率计算(skip-calc-frame-rate)
    ...
    // 根据名称查找输入码流格式,这里iformat_name未指定，不走这里
    if (ffp->iformat_name)
        is->iformat = av_find_input_format(ffp->iformat_name);

    // 打开码流获取header信息
    // https://blog.csdn.net/zzyincsdn/article/details/121458569
    av_log(NULL, AV_LOG_INFO, "read_thread > avformat_open_input start\n");
    err = avformat_open_input(&ic, is->filename, is->iformat, &ffp->format_opts);
    av_log(NULL, AV_LOG_INFO, "read_thread > avformat_open_input end\n");
    if (err < 0) {
    ...
    // avformat_open_input执行完毕关闭scan_all_pmts_set
    ...
    // ffp->find_stream_info默认为1
    if (ffp->find_stream_info) {
        av_log(NULL, AV_LOG_FATAL, "read_thread > nb_streams:%d\n",ic->nb_streams);
        AVDictionary **opts = setup_find_stream_info_opts(ic, ffp->codec_opts);
        int orig_nb_streams = ic->nb_streams;
        av_log(NULL, AV_LOG_FATAL, "read_thread > orig_nb_streams:%d\n",orig_nb_streams);
...
#endif
    av_register_all();
    av_log(NULL, AV_LOG_INFO, "ffp_global_init > ijkav_register_all call before\n");
    ijkav_register_all();
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name)
{
    ...
    // 初始化音频输出SDL_Aout,根据配置是使用AudioTrack还是opensles
    if (!ffp->aout) {
...
/**
 * @brief 音轨选择，如中英双语切换等
 * https://github.com/bilibili/ijkplayer/issues/4264
 * @param ffp 
 * @param stream 
 * @param selected 
 * @return int 
